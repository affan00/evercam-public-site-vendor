...
class Options_Machine {
		...
		foreach ($options as $value) {
			...
			//Start Heading
			 if ( $value['type'] != "heading" )
			 {
				...
				if($value['sub']) $output .= '<h4 class="heading">'. $value['sub'] .'</h4>'."\n";
			...
			switch ( $value['type'] ) {
				...
				case 'backup':
					...
					$output .= '<p><strong>'. __('Last Backup : ', 'tdjournal').'<span class="backup-log">'.$log.'</span></strong></p></div>'."\n";
				...
				//drag & drop recent blog posts manager
				case 'recent_posts':
					$output .= '<div class="slider"><ul id="'.$value['id'].'">';
					$slides = $smof_data[$value['id']];
					$count = count($slides);
					if ($count < 2) {
						$oldorder = 1;
						$order = 1;
						$output .= Options_Machine::optionsframework_recent_posts_function($value['id'],$value['std'],$oldorder,$order);
					} else {
						$i = 0;
						foreach ($slides as $slide) {
							$oldorder = $slide['order'];
					...
					$output .= '</ul>';
					$output .= '<a href="#" class="button posts_add_button">Add New</a></div>';
					
				break;
				
				//drag & drop manager for extended slider
				case 'slider_extended':
					$output .= '<div class="slider"><ul id="'.$value['id'].'">';
					$slides = $smof_data[$value['id']];
					$count = count($slides);
					if ($count < 2) {
						$oldorder = 1;
						$order = 1;
						$output .= Options_Machine::optionsframework_slider_extended_function($value['id'],$value['std'],$oldorder,$order);
					} else {
						$i = 0;
						foreach ($slides as $slide) {
							$oldorder = $slide['order'];
					...
					$output .= '</ul>';
					$output .= '<a href="#" class="button slider_extended_add_button">Add New</a></div>';
					
				break;
				
				//drag & drop manager home page slideshow
				case 'featured_slider':
					...
					if ($count < 2) {
						$oldorder = 1;
						$order = 1;
						$output .= Options_Machine::optionsframework_featured_slider_function($value['id'],$value['std'],$oldorder,$order);
					} else {
						$i = 0;
						foreach ($slides as $slide) {
							$oldorder = $slide['order'];
					...
					$output .= '</ul>';
					$output .= '<a href="#" class="button featured_slider_add_button">Add New</a></div>';
					
				break;
				
				//drag & drop manager simple slider
				case 'simple_slider':
					$output .= '<div class="slider"><ul id="'.$value['id'].'">';
					$slides = $smof_data[$value['id']];
					$count = count($slides);
					if ($count < 2) {
						$oldorder = 1;
						$order = 1;
						$output .= Options_Machine::optionsframework_simple_slider_function($value['id'],$value['std'],$oldorder,$order);
					} else {
						$i = 0;
						foreach ($slides as $slide) {
							$oldorder = $slide['order'];
					...
					$output .= '</ul>';
					$output .= '<a href="#" class="button simple_slider_add_button">Add New</a></div>';
	...
	public static function optionsframework_slider_function($id,$std,$oldorder,$order){
	    ...
	    if( !empty( $slide ) && count( $slide ) <=1 ) {
			$slide = array_combine(range(1, count($slide)), array_values($slide));
		}
	 ...
	 * @return string
	 */
	public static function optionsframework_slider_extended_function($id,$std,$oldorder,$order){
		...
		//initialize all vars
		$slidevars = array('title','url','link','cta','description', 'align');
		...
		$slider .= '<label>Link CTA (optional)</label>';
		$slider .= '<input class="slide of-input" name="'. $id .'['.$order.'][cta]" id="'. $id .'_'.$order .'_slide_cta" value="'. $val['cta'] .'" />';

		$slider .= '<label>Description (optional)</label>';
		$slider .= '<textarea class="slide of-input" name="'. $id .'['.$order.'][description]" id="'. $id .'_'.$order .'_slide_description" cols="8" rows="8">'.stripslashes($val['description']).'</textarea>';
	
		$slider .= '<label>Text Align</label>';
		$slider .= '<div class="option"><div class="controls"><div class="select_wrapper mini" style="width: 120px;">
						<select class="select of-input" name="'. $id .'['.$order.'][align]" id="'. $id .'_'.$order .'_slide_align">
							<option value="" '.selected( '', $val['align'], false ).'>Left</option>
							<option value="right" '.selected( 'right', $val['align'], false ).'>Right</option>
							<option value="center" '.selected( 'center', $val['align'], false ).'>Center</option>
						</select>
					</div></div></div>';
	...
	/**
	 * Drag and drop slides manager for recent blog posts
	 *
	 * @uses get_theme_mod()
	 *
	 * @access public
	 ...
	 * @return string
	 */
	public static function optionsframework_recent_posts_function($id,$std,$oldorder,$order) {
		...
		//initialize all vars
		$slidevars = array('title', 'cat_id', 'per_page', 'summary', 'post_layout');
		...
		//Per Page Value 
		if( !empty( $val['per_page'] ) ) {
			$per_page_number = intval($val['per_page']);
		} else {
			$per_page_number = 1;
		}
		...
		if (!empty($val['title'])) {
			...
			$slider .= '<li><div class="slide_header"><strong>Section '.$order.'</strong>';
		}
		...
		$slider .= '<label>Category ID</label>';
		$slider .= '<input class="slide of-input of-slider-cat-id" name="'. $id .'['.$order.'][cat_id]" id="'. $id .'_'.$order .'_slide_cat_id" value="'. $val['cat_id'] .'" />';
		
		$slider .= '<label>Posts Per Page</label>';
		$slider .= '<input class="slide of-input of-slider-per-page" name="'. $id .'['.$order.'][per_page]" id="'. $id .'_'.$order .'_slide_per_page" value="'. $per_page_number .'" />';
		
		$slider .= '<label>Post Summary</label>';
		$slider .= '<div class="option"><div class="controls"><div class="select_wrapper mini" style="width: 120px;">
						<select class="select of-input" name="'. $id .'['.$order.'][summary]" id="'. $id .'_'.$order .'_slide_summary">
							<option value="" '.selected( '', $val['summary'], false ).'>Off</option>
							<option value="1" '.selected( '1', $val['summary'], false ).'>On</option>
						</select>
					</div></div></div>';
		
		$slider .= '<label>Layout</label>';
		$slider .= '<div class="option"><div class="controls"><div class="select_wrapper mini" style="width: 220px;">
						<select class="select of-input" name="'. $id .'['.$order.'][post_layout]" id="'. $id .'_'.$order .'_slide_post_layout">
							<option value="one-column" '.selected( 'one-column', $val['post_layout'], false ).'>One Column</option>
							<option value="one-column-alt" '.selected( 'one-column-alt', $val['post_layout'], false ).'>One Column (Alt)</option>
							<option value="two-columns" '.selected( 'two-columns', $val['post_layout'], false ).'>Two Columns</option>
							<option value="three-columns" '.selected( 'three-columns', $val['post_layout'], false ).'>Three Columns</option>
							<option value="standard" '.selected( 'standard', $val['post_layout'], false ).'>Standard</option>
						</select>
					</div></div></div>';
	 ...
	 * @return string
	 */
	public static function optionsframework_featured_slider_function($id,$std,$oldorder,$order){
		...
		//initialize all vars
		$slidevars = array('title','url','link','cta','description','caption_align','embed_video');
		...
		$slider .= '<label>Embed Video</label>';
		$slider .= '<textarea class="slide of-input" name="'. $id .'['.$order.'][embed_video]" id="'. $id .'_'.$order .'_slide_embed_video" cols="8" rows="8">'.stripslashes($val['embed_video']).'</textarea>';
		...
		$slider .= '<label>Caption</label>';
		$slider .= '<div class="option"><div class="controls"><div class="select_wrapper mini" style="width: 120px;">
						<select class="select of-input" name="'. $id .'['.$order.'][caption_align]" id="'. $id .'_'.$order .'_slide_caption_align">
							<option value="" '.selected( '', $val['caption_align'], false ).'>Left Align</option>
							<option value="right" '.selected( 'right', $val['caption_align'], false ).'>Right Align</option>
							<option value="center" '.selected( 'center', $val['caption_align'], false ).'>Full Width</option>
							<option value="none" '.selected( 'none', $val['caption_align'], false ).'>None</option>
						</select>
					</div></div></div>';
	...
	/**
	 * Drag and drop simple slides manager
	 *
	 * @uses get_theme_mod()
	 *
	 * @access public
	 ...
	 * @return string
	 */
	public static function optionsframework_simple_slider_function($id,$std,$oldorder,$order){
